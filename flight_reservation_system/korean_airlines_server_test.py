...
# 편도편 조회
flight_ids1, flight_infos1 = korean_air.proxy_server.search_flight_schedule_one_way("2024-07-01", "인천(ICN)", "토론토(YYZ)")
flight_ids2, flight_infos2 = korean_air.proxy_server.search_flight_schedule_one_way("2024-07-07", "토론토(YYZ)", "인천(ICN)") # 반환 값 : None, None (검색 조건에 일치하는 결과 없음)
...
# 편도 에약
t_or_f, booking_reference1 = korean_air.proxy_server.book_flight(flight_id1, None, 2, passengers_info, "goodtrip")

print("예약 성공 여부 : ", str(t_or_f)) # 좌석수가 부족해 실패
print("예약 번호 : ", str(booking_reference1)) 

# 왕복 예약
t_or_f, booking_reference2 = korean_air.proxy_server.book_flight(flight_id1, flight_id2, 2, passengers_info, "goodtrip")

print("예약 성공 여부 : ", str(t_or_f)) # 가는편의 좌석수가 부족해 실패
